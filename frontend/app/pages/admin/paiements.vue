        ...
        <template v-else>
          ...
          <div class="card p-4 space-y-3">
            ...
            <div v-else-if="paiements.length" class="space-y-2">
              <div
                v-for="p in paiements" :key="p.id"
                class="flex items-center justify-between p-3 rounded-xl bg-white/3 border border-white/5"
              >
                <div>
                  <p class="text-sm font-semibold text-green-400">+{{ p.montant }} DH</p>
                  <p class="text-xs text-app-text/40">{{ p.mode_paiement }} · {{ formatDate(p.date_paiement) }}</p>
                </div>
                <span class="text-xs text-green-400 bg-green-400/10 px-2 py-0.5 rounded-full">Payé</span>
              </div>
            </div>
