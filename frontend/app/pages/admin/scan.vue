...
function clearFile(): void {
  file.value = null; preview.value = null; isPdf.value = false
}
  ...
  try {
    const res = await $fetch<{ success: boolean; data: any }>(
      `${getApiBase()}/api/document-types`,
      {
        ...
        body: { name: newTypeName.value.trim(), has_expiration: newTypeExpires.value, is_required: false },
      }
    )
    documentTypes.value.push(res.data)
    form.document_type_id = res.data.id
    showNewType.value = false; newTypeName.value = ''; newTypeExpires.value = false
  ...
  <div class="space-y-5 animate-fade-up">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
      ...
      <div class="card p-5 space-y-4">
        ...
        <label class="block rounded-xl border-2 border-dashed border-white/20 p-8 text-center cursor-pointer hover:border-gold/40 transition"
      ...
      <div class="card p-5 space-y-4">
        ...
        <div>
          ...
          <select v-model="form.entreprise_id" class="f-input">
            ...
            <option v-for="c in clientItems" :key="c.id" :value="c.id">{{ c.raison_sociale }}</option>
          </select>
        </div>
        ...
        <div>
          <div class="flex items-center justify-between mb-1">
            ...
            <button class="text-xs text-gold underline" @click="showNewType = !showNewType">
          ...
          <div v-else class="rounded-xl border border-gold/20 bg-gold/5 p-3 space-y-2">
            ...
            <button class="btn btn-gold btn-sm w-full" :disabled="creatingType || !newTypeName.trim()"
                    @click="createNewType">
        ...
        <button class="btn btn-gold btn-lg w-full justify-center mt-2"
