 ...
 *
 * FIX (this revision):
 *   Step 4 previously called openPreview() and buildStreamUrl(), two
 *   functions that no longer exist after the wizard was migrated to the
 *   shared ContratPreviewModal component. That threw a ReferenceError the
 *   instant Vue tried to render Step 4 — right after the "contract saved"
 *   toast fired — so the view silently failed to advance past Step 3.
 *   Both call sites now use the functions that actually exist:
 *     - "Aperçu"      → openSavedPreview()  (opens ContratPreviewModal)
 *     - "Télécharger" → contratService.streamPdfUrl(id, 'download')
 */
 ...
 * This is the correct handler for Step 4's "Aperçu" button; the old
 * openPreview()/showPreview local-modal pair no longer exists.
 */
  ...
  <div class="space-y-5 animate-fade-up max-w-3xl mx-auto">
      ...
      <div class="card p-5 space-y-4">
        ...
        <div>
          ...
          <p class="text-[10px] mt-1" style="color:var(--app-text-faint)">
            Ce texte apparaît centré en tête du document. Laissez vide pour utiliser
    ...
    <div v-else-if="step === 3" class="space-y-5">
      ...
      <div v-else class="rounded-xl p-8 text-center"
        ...
        <p class="text-sm">Le document sera généré sans articles de contrat.</p>
      </div>
    ...
    <div v-else-if="step === 4" class="space-y-4">
      <div class="card p-6 text-center space-y-4">
        ...
        <!--
          FIX: these two controls previously called openPreview() and
          buildStreamUrl('download'), neither of which exists in this
          version of the script — that threw a ReferenceError the moment
          Step 4 tried to render, which is why the wizard appeared stuck
          on Step 3 right after the "saved" toast.

          openSavedPreview()  → opens ContratPreviewModal against the
                                 saved contract's live preview stream.
          contratService.streamPdfUrl(id, 'download') → direct download
                                 link for the same document, PDF mode.
        -->
