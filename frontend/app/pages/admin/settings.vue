...
function saveDelaysToStorage(delays: number[]): void {
  if (!import.meta.client) return
  localStorage.setItem('app_notif_delays', JSON.stringify(delays))
}
...
<template>
  <div class="space-y-6 animate-fade-up max-w-2xl">
    ...
    <div class="card p-5 space-y-4">
      <div>
        <p class="text-xs uppercase text-gold tracking-widest font-bold">🔔 Alertes d'expiration de contrat</p>
      ...
      <div v-else class="flex gap-3 flex-wrap">
        <button v-for="m in [1, 3, 6]" :key="m"
                ...
                @click="toggleDelay(m)">
          {{ m }} mois avant <span v-if="selectedDelays.includes(m)" class="ml-1">✓</span>
        </button>
      </div>
      ...
      <button class="btn btn-gold btn-md" :disabled="savingPrefs || loadingPrefs" @click="savePreferences">
