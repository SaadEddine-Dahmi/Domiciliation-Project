...
function getRawToken(): string {
  ...
  try {
    // FIX: was reading 'astfisc_auth' — every other page writes/reads
    // 'app_auth', so document preview/download on this page silently 401'd.
...
function authHeaders(): Record<string, string> {
  ...
  try {
    // FIX: same key mismatch as getRawToken() above.
    const raw = localStorage.getItem('app_auth')
