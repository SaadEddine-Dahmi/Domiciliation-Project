...
//     Routes that a browser opens via <iframe src> or <a href> — the
//     browser cannot attach an Authorization header to these requests.
//     Each controller validates access through an alternative mechanism
//     (query-param token, or accepts public read for non-sensitive
//     previews).
//
//   AUTHENTICATED (auth:sanctum):
//     All other routes. Require a valid Bearer token in the
...
// ── Public file endpoints (browser-initiated, no Authorization header) ──
...
// Contract PDF stream — used by the preview <iframe> in the wizard.
// MUST remain outside auth:sanctum for the same reason as document
// endpoints. ContratController::streamPdf() streams the stored file or
// renders on-the-fly.
Route::get('/contrats/{id}/pdf/stream', [ContratController::class, 'streamPdf'])
    ->name('contrats.pdf.stream');

// Invoice PDF — same pattern as document preview.
...
Route::middleware(['auth:sanctum', 'throttle:api'])->group(function () {
    ...
    // Renewal — creates a brand new draft contract linked to the source.
    // POST is correct here because it creates a resource.
    Route::post('/contrats/{id}/renew', [ContratController::class, 'renew'])
        ->middleware('throttle:heavy');

    // PDF generation — saves to disk. Heavy rate limit protects server
    // resources against repeated dompdf renders.
    Route::post('/contrats/{id}/pdf', [ContratController::class, 'generatePdf'])
        ->middleware('throttle:heavy');

    // NOTE: GET /contrats/{id}/pdf/stream is registered ABOVE, outside
    // this group, since it must be reachable without an Authorization
    // header.
